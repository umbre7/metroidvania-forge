...
const DOOR_SWITCH_AUDIO = preload("uid://uedlunksokyi")
...
func _ready() -> void:
	if SaveManager.persistent_data.get_or_add(unique_name(), "close") == "open":
		set_open()
	else:
...
func _on_player_interacted(_p : Player) -> void:
